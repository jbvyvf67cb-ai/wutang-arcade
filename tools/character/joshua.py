...
def build_mesh() -> MeshData:
    ...
    # ---- arms: permanent elbow bend so the joint reads in silhouette ----
    for sx in (1, -1):
        # bicep: shoulder down to a forward-set elbow
        add_tube(md, [
            S((sx * 0.27, 0.01, 1.44), 0.120, 0.120),
            S((sx * 0.37, 0.00, 1.28), 0.108, 0.108),
            S((sx * 0.42, -0.05, 1.15), 0.092, 0.092),
        ], n=10, tag="arm")
        # elbow ball (slight bulge at the joint)
        add_sphere(md, (sx * 0.425, -0.055, 1.135), 0.085, tag="arm")
        # forearm: angles forward from the elbow, slimmer than the bicep
        add_tube(md, [
            S((sx * 0.425, -0.06, 1.13), 0.080, 0.080),
            S((sx * 0.45, -0.11, 1.01), 0.072, 0.072),
            S((sx * 0.465, -0.13, 0.94), 0.068, 0.068),
        ], n=10, tag="arm")
        # paw (hand)
        add_tube(md, [
            S((sx * 0.47, -0.13, 0.95), 0.095, 0.082),
            S((sx * 0.475, -0.15, 0.79), 0.110, 0.070),
        ], n=8, tag="paw")
        # 4 claws per hand, bases embedded in the paw
        for i, ox in enumerate((-0.055, -0.018, 0.018, 0.055)):
            add_cone(
                md,
                (sx * 0.475 + ox, -0.155, 0.81),
                (sx * 0.475 + ox * 1.3, -0.21, 0.71),
                0.016,
            )
...
BONES = [
    ...
    ("upper_arm.L", (0.29, 0.01, 1.43), (0.425, -0.055, 1.13), "spine"),
    ("forearm.L", (0.425, -0.055, 1.13), (0.47, -0.13, 0.94), "upper_arm.L"),
    ("hand.L", (0.47, -0.13, 0.94), (0.478, -0.17, 0.74), "forearm.L"),
    ("upper_arm.R", (-0.29, 0.01, 1.43), (-0.425, -0.055, 1.13), "spine"),
    ("forearm.R", (-0.425, -0.055, 1.13), (-0.47, -0.13, 0.94), "upper_arm.R"),
    ("hand.R", (-0.47, -0.13, 0.94), (-0.478, -0.17, 0.74), "forearm.R"),
